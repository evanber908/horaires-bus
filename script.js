...
function filterHoraires() {
  ...
  if (isToday) {
    ...
    const departs0 = departsDuJour(arret, dateChoisie)
      .filter(d => destinationChoisie === "" || d.dest === destinationChoisie);

    departs = departs0.map(d => ({ ...d, isDemain: false, isOtherDay: true, dateObj: dateChoisie }));
  }
  ...
  // On scrolle à l'endroit désiré UNIQUEMENT une fois au clic/chargement de la liste
  if (premierBusActif && scheduleList) {
    setTimeout(() => {
      ...
      scheduleList.scrollTo({
        top: scheduleList.scrollTop + (itemRect.top - listRect.top) - 10,
        behavior: 'smooth'
      });
    }, 100);
  }
...
function findNextBus(userLat, userLng) {
  ...
  if (prochains.length === 0) {
    resultat.innerHTML = `<strong>${nomArret(arretProche)}</strong> : Aucun bus dans les 24h.`;
  } else {
    const p = prochains[0];
    const status = p.isDemain ? 'Demain' : 'Aujourd\'hui';
    resultat.innerHTML = `📍 Le plus proche : <strong>${nomArret(arretProche)}</strong><br>🚌 Prochain départ vers ${p.dest} à <strong>${p.h}</strong> (${status})`;
  }
}
...
async function init() {
  try {
    const [repLignes, repHoraires] = await Promise.all([
      ...
      fetch(CONFIG.horaires).then(r => r.json())
    ]);

    REF = repHoraires;
    
    // Garde ton réglage pour supprimer les 5 premiers arrêts
